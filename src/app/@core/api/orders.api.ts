...
export class OrdersApi {
  ...
  list(pageNumber: number = 1, pageSize: number = 10): Observable<any[]> {
    ...
    return this.api.get(this.apiController, { params })
      .pipe(map(data => data.map(item => {
        const picture = `${this.api.apiUrl}/${this.apiController}/${item.tradeno}/photo`;
        return { ...item, picture };
      })));
  }
  ...
  getCurrent(): Observable<any> {
    return this.api.get(`${this.apiController}/current`)
      .pipe(map(data => {
        const picture = `${this.api.apiUrl}/${this.apiController}/${data.tradeno}/photo`;
        return { ...data, picture };
      }));
  }

  get(tradeno: string): Observable<any> {
    return this.api.get(`${this.apiController}/${tradeno}`)
      .pipe(map(data => {
        const picture = `${this.api.apiUrl}/${this.apiController}/${data.tradeno}/photo`;
        return { ...data, picture };
      }));
  ...
  delete(tradeno: string): Observable<boolean> {
    return this.api.delete(`${this.apiController}/delete/${tradeno}`);
  }
  ...
  update(item: any): Observable<any> {
    return this.api.put(`${this.apiController}/update/${item.tradeno}`, item);
  }
}
